...
The experiment uses two hypothetical 3' UTRs based on the longest Rac1 transcript. Parameters sweeped over include the truncated transcript sizes (350-600 nt), distance between cleavage sites (50-700 nt), and read counts per UTR (50 or 100 reads).
  ...
  for (proxCounts in c(50, 100)) {
    for (distCounts in c(50, 100)) {
      for (txWidth in seq(350, 600, 50)) {
        f <- sprintf("../data/kallisto/Rac1.d%d.ctp%d.ctd%d.txw%d/confusion.txt", 
                     d, proxCounts, distCounts, txWidth)
        df_i <- read_fwf(f, fwf_widths(c(7, 5, NA), c("counts", "origin", "result"))) %>%
          mutate(distance=d, transcriptWidth=txWidth,
...
df.pseudo <- df.pseudo %>% 
  complete(origin, result, distance, transcriptWidth, proximalCounts, 
           distalCounts, fill=list(counts=0))
```
...
for (d in seq(50, 700, 50)) {
  for (proxCounts in c(50, 100)) {
    for (distCounts in c(50, 100)) {
      for (txWidth in seq(350, 600, 50)) {
        f <- sprintf("../data/kallisto/Rac1.d%d.ctp%d.ctd%d.txw%d/abundance.tsv", 
                     d, proxCounts, distCounts, txWidth)
